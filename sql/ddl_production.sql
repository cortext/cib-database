...
CREATE TABLE `cib_firm_names`
             (
                          `guo_orbis_id`       VARCHAR(16) NOT NULL,
                          `previous_name`      VARCHAR(255) DEFAULT NULL,
                          `previous_name_date` VARCHAR(50) DEFAULT NULL,
                          `aka_name`           VARCHAR(255) DEFAULT NULL,
                          KEY `cib_firm_names_guo_orbis_id_idx` (`guo_orbis_id`) using btree,
                          CONSTRAINT `cib_firm_names_fk` FOREIGN KEY (`guo_orbis_id`) REFERENCES `cib_firms` (`guo_orbis_id`)
             )
             engine=innodb DEFAULT charset=utf8 COLLATE=utf8_unicode_ci;
